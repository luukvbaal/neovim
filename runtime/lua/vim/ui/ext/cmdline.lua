...
M.cmdline_show = function(content, pos, firstc, prompt, indent)
  ...
  if ext.cmdheight == 0 and api.nvim_win_get_config(ext.wins[ext.tab].cmd).hide then
    api.nvim_win_set_config(ext.wins[ext.tab].cmd, { hide = false, height = height })
    ext.msg_set_pos(false, height - (ext.cmdheight == 0 and 1 or 0), 0)
...
M.cmdline_pos = function(pos)
  ...
  api.nvim__redraw({ win = ext.wins[ext.tab].cmd, cursor = true })
end
