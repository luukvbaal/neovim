...
api.nvim_create_autocmd('OptionSet', {
  ...
  callback = function(ev)
    if ev.match == 'cmdheight' then
      ...
      api.nvim_win_set_config(M.wins[M.tab].cmd, { hide = M.cmdheight == 0 })
...
local function ls_height(closedid)
  if M.cmdheight == 0 and M.cmdline then
    return 1
  end
...
M.msg_set_pos = function(newheight, cmdheight, closedid)
  ...
  if M.msg.count == 0 or not api.nvim_win_is_valid(win) then
    return -- No need to set pos when there are no messages.
  end
  ...
  if h <= math.ceil(vim.o.lines * 0.5) then
    api.nvim_win_set_config(win, {
      ...
      col = vim.o.columns - api.nvim_win_get_width(win),
    })
  else
    M.msg.msg_to_split()
  end
